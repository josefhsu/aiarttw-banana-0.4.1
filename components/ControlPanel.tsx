...
import { dataURLtoFile, getImageDimensions, getAspectRatio, cropImageToAspectRatio } from '../utils';
...
type ControlPanelProps = {
    ...
    drawAspectRatio: AspectRatio | null;
    ...
    veoAspectRatio: VeoAspectRatio | null;
...
const WebcamCapture: React.FC<{
...
}> = ({ onClose, onImageSelect, aspectRatio }) => {
    ...
    const calculateOverlay = useCallback(() => {
        ...
        if (videoRatio > targetRatio) { // Video is wider than target
            const overlayWidth = videoHeight * targetRatio;
            setOverlayStyle({
                width: `${overlayWidth}px`,
                height: '100%',
                top: 0,
                left: `${(videoWidth - overlayWidth) / 2}px`,
            });
        } else { // Video is taller than target
            const overlayHeight = videoWidth / targetRatio;
            setOverlayStyle({
                width: '100%',
                height: `${overlayHeight}px`,
                left: 0,
                top: `${(videoHeight - overlayHeight) / 2}px`,
            });
        }
    }, [aspectRatio]);
    ...
    const handleTakePhoto = useCallback(async () => {
        if (videoRef.current && canvasRef.current) {
            ...
            if (imageData) {
                ...
                try {
                    setProcessingMessage('正在裁切圖片');
                    const croppedData = await cropImageToAspectRatio(imageData, aspectRatio);
                    const capturedFile = dataURLtoFile(croppedData, `webcam-capture-${Date.now()}.png`);
                    
                    setProcessingMessage('正在優化光線');
                    const enhancedBase64 = await geminiService.enhanceWebcamImage(capturedFile);
                    const enhancedFile = dataURLtoFile(`data:image/png;base64,${enhancedBase64}`, `webcam-enhanced.png`);

                    setProcessingMessage('正在移除背景');
                    const processedBase64 = await removeBackground(enhancedFile, true);
    ...
    }, [videoRef, canvasRef, onImageSelect, onClose, aspectRatio]);
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center" onClick={onClose}>
            <div className="themed-panel p-4 rounded-lg shadow-lg max-w-lg w-full" onClick={e => e.stopPropagation()}>
                <h3 className="text-lg font-bold mb-4">攝像頭拍攝 (將裁切為 {aspectRatio})</h3>
                <div className="relative w-full aspect-video bg-black rounded-md overflow-hidden flex items-center justify-center">
                    {(isLoading || isProcessing) && (
                         <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50 z-20">
                            ...
                            <p className="font-semibold">{isProcessing ? processingMessage : '正在啟動攝像頭...'}</p>
                         </div>
                    )}
                    ...
                    {!isLoading && (
                         <div 
                             ...
                             style={{ ...overlayStyle, borderColor: 'rgba(0, 255, 255, 0.7)', boxShadow: '0 0 10px rgba(0, 255, 255, 0.5)' }}
                         ></div>
                    )}
                </div>
                 <button onClick={handleTakePhoto} disabled={isLoading || isProcessing} className="w-full mt-4 py-2 themed-button rounded-lg font-semibold flex items-center justify-center gap-2">
                    {isProcessing ? `${processingMessage}...` : '拍攝照片 (Enter)'}
                </button>
            </div>
        </div>
    );
...
const VersionInfo: React.FC<{ modifierKey: 'Ctrl' | '⌘' }> = ({ modifierKey }) => (
    <div className="flex flex-col h-full text-slate-400 p-4 overflow-y-auto">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
            <img 
                src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgADIIYcccsghhxyfc/x/AQYAcelI+bfRq7wAAAAASUVORK5CYII="
                alt="鳥巢AI包娜娜 Logo" 
...
const MultiUploader: React.FC<{
    ...
    aspectRatio: AspectRatio | VeoAspectRatio | null;
}> = ({ images, onFilesUpload, onRemoveImage, maxFiles, placeholder, borderColorClass, aspectRatio }) => {
                ...
                const promise = new Promise<UploadedImage>((resolve, reject) => {
                    reader.onload = async (e) => {
                        try {
                            ...
                            if (aspectRatio) {
                                src = await cropImageToAspectRatio(src, aspectRatio);
                            }
                            const finalFile = dataURLtoFile(src, file.name);
                            const { width, height } = await getImageDimensions(src);
                            resolve({ src, file: finalFile, width, height, id: crypto.randomUUID() });
                        } catch(err) {
                            console.error("Could not process image for upload", err);
                            reject(err);
                        }
                    };
    ...
    return (
        <div>
                 ...
                 {images.length === 0 ? (
                    ...
                    <div className={`grid gap-2 ${maxFiles > 1 ? 'grid-cols-4' : 'grid-cols-1'}`}>
                        {images.map((img, index) => (
                            <div 
                                key={img.id || index}
                                className="relative group rounded-md overflow-hidden bg-black/20"
                                style={{ aspectRatio: (img.width && img.height) ? `${img.width} / ${img.height}` : '1 / 1' }}
                            >
                                <img src={img.src} alt={`upload-${index}`} className="w-full h-full object-cover"/>
                                {img.isProcessing && (
            ...
            {isWebcamOpen && aspectRatio && <WebcamCapture aspectRatio={aspectRatio} onClose={() => setIsWebcamOpen(false)} onImageSelect={(img) => onFilesUpload([img])} />}
        </div>
    );
};
...
const DisabledUploadPlaceholder: React.FC = () => (
    <div className="text-center p-4 border-2 border-dashed border-fuchsia-500/50 rounded-md min-h-[100px] flex items-center justify-center">
        <p className="text-fuchsia-300 text-sm">請先在上方選擇長寬比例</p>
    </div>
);
...
export const ControlPanel: React.FC<ControlPanelProps> = (props) => {
    ...
    const renderGeneratePanel = () => (
        <>
            <Section title="1. 圖片設定">
                <div className="space-y-3">
                     <div>
                        <label className="text-xs text-slate-400 block mb-1">長寬比例 (必填)</label>
            ...
            <Section title="3. 參考圖 (選填，最多8張)" className="relative">
                 {isSuggestingEdit && (
                 ...
                 {!props.selectedAspectRatio ? (
                    <DisabledUploadPlaceholder />
                 ) : (
                    <MultiUploader
                        images={props.referenceImages.filter(img => !img.isPlaceholder)}
                        ...
                        placeholder={
                            <div className="text-center">
                                ...
                                <p className="text-xs text-slate-600">自動裁切為 {props.selectedAspectRatio}</p>
                            </div>
                        }
                    />
                 )}
            ...
            <button onClick={() => props.onGenerate()} disabled={props.isLoading || isSuggestingEdit || !props.selectedAspectRatio} className="w-full py-3 mt-auto themed-button themed-button-shine rounded-lg font-semibold flex items-center justify-center gap-2" title={!isMobile ? `生成圖片 (${modifierKey}+Enter)` : '生成圖片'}>
        ...
        return (
            <>
                    ...
                    <Section title="III. 武器庫 & 載具" noMb>
                        ...
                        {!props.selectedAspectRatio ? (
                            ...
                            <MultiUploader
                                ...
                                borderColorClass="border-fuchsia-500/50"
                                aspectRatio={props.selectedAspectRatio}
                                placeholder={<p className="text-xs text-slate-500">點擊或拖曳武器圖片 (裁切為 {props.selectedAspectRatio})</p>}
                            />
                        )}
                         ...
                         {!props.selectedAspectRatio ? (
                            <DisabledUploadPlaceholder />
                             ...
                             <MultiUploader
                                ...
                                borderColorClass="border-fuchsia-500/50"
                                aspectRatio={props.selectedAspectRatio}
                                placeholder={<p className="text-xs text-slate-500">點擊或拖曳夥伴圖片 (裁切為 {props.selectedAspectRatio})</p>}
                            />
                         )}
                ...
                <div className="mt-auto space-y-2">
                    <button onClick={props.onRandomSceneGenerate} disabled={props.isLoading || !props.selectedAspectRatio} className="w-full py-3 themed-button themed-button-secondary rounded-lg font-semibold flex items-center justify-center gap-2">
                        隨選5場景
                    </button>
                    <div className="flex items-stretch gap-2">
                        ...
                        <button onClick={() => props.onGenerate()} disabled={props.isLoading || !props.selectedAspectRatio} className="flex-1 py-3 themed-button themed-button-shine rounded-lg font-semibold flex items-center justify-center gap-2">
    ...
    const renderRemoveBgPanel = () => (
        <>
            <Section title="1. 圖片設定">
                <div className="space-y-3">
                     <div>
                        <label className="text-xs text-slate-400 block mb-1">裁切比例 (必填)</label>
                        <div className="grid grid-cols-5 gap-2 text-xs">
                           {ASPECT_RATIOS.map(ratio => (
                               <button 
                                key={ratio} 
    ...
    const renderDrawPanel = () => {
        ...
        return (
            <>
                 <Section title="畫布">
                    <div className="mb-3">
                        <label className="text-xs text-slate-400 block mb-1">畫布比例 (必填)</label>
                        <div className="grid grid-cols-5 gap-2 text-xs">
                           {ASPECT_RATIOS.map(ratio => (
                               <button 
                                key={ratio} 
                    ...
                    {!props.drawAspectRatio ? (
                        ...
                        <MultiUploader
                            ...
                            maxFiles={1}
                            borderColorClass="border-fuchsia-500/50"
                            aspectRatio={props.drawAspectRatio}
                            placeholder={
                                <div className="text-center text-xs text-slate-400 py-4">
                                    <p>上傳新背景 (選填)</p>
                                    <p className="text-slate-600">自動裁切為 {props.drawAspectRatio}</p>
                                </div>
                            }
                        />
                    )}
                ...
                <div className="grid grid-cols-3 gap-2 mt-auto">
                    ...
                    <button onClick={props.onUseDrawing} disabled={!props.drawAspectRatio} className="col-span-3 py-3 themed-button themed-button-shine rounded-lg font-semibold" title={!isMobile ? `使用此畫布生成 (${modifierKey}+Enter)` : '使用此畫布生成'}>
                        使用此畫布生成
                    </button>
                </div>
            </>
    ...
    const VeoFrameUploader: React.FC<{
        ...
        disabled?: boolean;
    }> = ({ label, image, setImage, aspectRatio, disabled }) => {
        ...
        const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (file?.type.startsWith('image/') && aspectRatio) {
                ...
                reader.onload = async (e) => {
                    ...
                    const croppedSrc = await cropImageToAspectRatio(src, aspectRatio);
                    const croppedFile = dataURLtoFile(croppedSrc, file.name);
                    const { width, height } = await getImageDimensions(croppedSrc);
                    setImage({ src: croppedSrc, file: croppedFile, width, height });
                };
                reader.readAsDataURL(file);
            }
        ...
        const isDisabled = disabled || !aspectRatio;
            ...
            <div className="relative">
                ...
                {image ? (
                     <div 
                        ...
                        className={`relative group aspect-video rounded-md overflow-hidden ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                        ...
                        <button onClick={(e) => { e.stopPropagation(); if(!isDisabled) setImage(null); }} disabled={isDisabled} className="absolute top-1 right-1 p-0.5 bg-black/60 rounded-full text-white opacity-0 group-hover:opacity-100"><XIcon className="w-3 h-3"/></button>
                    </div>
                ) : (
                    <div onClick={() => !isDisabled && setIsMenuOpen(true)} className={`flex flex-col items-center justify-center w-full h-full aspect-video bg-black/30 rounded-lg border-2 border-dashed border-fuchsia-500/50 ${isDisabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-fuchsia-500'}`}>
                        <PlusIcon className="w-6 h-6 text-slate-500" />
                        <p className="text-xs text-slate-500 mt-1">{aspectRatio ? '點擊上傳' : '先選比例'}</p>
                    </div>
                )}
                {isMenuOpen && (
                ...
                {isWebcamOpen && aspectRatio && <WebcamCapture onClose={() => setIsWebcamOpen(false)} onImageSelect={handleImageSelect} aspectRatio={aspectRatio} />}
            </div>
        );
    };
    ...
    const renderVeoPanel = () => {
        return (
            <>
                <Section title="影片設定">
                    <div>
                        <label className="text-xs text-slate-400 block mb-1">寬高比 (必填)</label>
                ...
                <Section title="AI 導演 (將自動裁切)">
                    <div className="grid grid-cols-2 gap-3">
                        <VeoFrameUploader label="首幀 (可選)" image={props.startFrame} setImage={props.onStartFrameChange} aspectRatio={props.veoAspectRatio} disabled={props.isAnalyzingFrames} />
                        <VeoFrameUploader label="尾幀 (可選)" image={props.endFrame} setImage={props.onEndFrameChange} aspectRatio={props.veoAspectRatio} disabled={props.isAnalyzingFrames} />
                    </div>
                </Section>
                

                <button onClick={() => props.onGenerateVeo()} disabled={props.isGeneratingVideo || props.isAnalyzingFrames || !props.veoAspectRatio} className="w-full py-3 mt-auto themed-button themed-button-shine rounded-lg font-semibold flex items-center justify-center gap-2">
